...
$router->group(['middleware' => 'auth', 'prefix' => 'orders'], function () use ($router) {
    ...
    $router->post('/', [
        'middleware' => ['role:admin','role:cliente'],
        'uses' => 'OrderController@store']);
});

$router->group(['prefix' => 'task', 'middleware' =>  ['role:admin|cliente']],  function () use ($router) {
    $router->get('/{id}', 'ProductController@show');
}); 
$router->post('identity/signin', 'IdentityController@signin');
$router->get('identity/test', 'IdentityController@test');
$router->get('identity/agregarUsuario', 'IdentityController@test');
